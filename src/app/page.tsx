  ...
  if (!mounted) {
    return (
      <div className="page-container flex items-center justify-center min-h-screen">
        <div className="animate-pulse-soft"><Car className="w-12 h-12 text-[#D4A855]" /></div>
      </div>
    );
  }
    ...
    <div className="page-container pb-28">
      ...
      <div className="mb-5 animate-slide-up flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#f4c3af] to-[#e0a98b] p-0.5 shadow-lg shadow-[#f4c3af]/20">
              <div className="w-full h-full rounded-full bg-[#090d16] flex items-center justify-center text-[#D4A855] font-bold text-base">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[10px] text-[#D4A855] font-bold bg-cyan-500/15 border border-cyan-500/30 px-2.5 py-0.5 rounded-full flex items-center gap-1">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/[0.06] hover:bg-white/10 border border-white/10 text-xs font-bold text-white transition-all cursor-pointer"
        >
          <span>Đăng xuất</span>
            ...
            <div className="flex items-center gap-2">
              {user && ['tcth', 'admin'].includes(user.role) && (
                <button
                  ...
                  className="px-3 py-2 rounded-2xl bg-slate-950 text-[#D4A855] font-bold text-xs shadow-lg hover:scale-105 transition-all flex items-center gap-1.5 cursor-pointer"
                  title="Giao việc trực tiếp cho tài xế (CHỈ dành cho Phòng TCTH / Admin)"
                >
                  <Send className="w-4 h-4 text-[#D4A855]" />
              ...
              {user?.role === 'director' ? (
                ...
                >
                  <ClipboardList className="w-4 h-4 text-[#D4A855]" />
                  <span>Báo cáo vận hành</span>
                </button>
              ) : (
                <Link href="/new" className="w-10 h-10 rounded-full bg-slate-950 text-[#D4A855] flex items-center justify-center shadow-lg hover:scale-105 transition-transform" title="Tạo đề xuất mới">
      ...
      <div className="mb-6 animate-slide-up overflow-x-auto no-scrollbar" style={{ animationDelay: '0.08s' }}>
        <div className="flex items-center gap-2.5" style={{ minWidth: 'max-content' }}>
          {user && ['tcth', 'admin'].includes(user.role) && (
            <button
              onClick={() => setShowDirectTaskModal(true)}
              className="flex items-center gap-2 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/40 hover:border-cyan-400 px-4 py-2.5 rounded-full text-xs font-bold text-[#D4A855] transition-all shadow-lg shadow-cyan-500/10 cursor-pointer"
            >
              <Send className="w-4 h-4 text-[#D4A855]" />
              <span>Giao việc trực tiếp</span>
            </button>
          ...
          {user?.role === 'director' ? (
            <button
              onClick={() => setShowOpsReport(true)}
              className="flex items-center gap-2 bg-gradient-to-r from-emerald-500/20 to-teal-500/20 border border-emerald-500/40 hover:border-emerald-400 px-4 py-2.5 rounded-full text-xs font-bold text-[#D4A855] transition-all shadow-lg shadow-emerald-500/10 cursor-pointer"
            >
              <ClipboardList className="w-4 h-4 text-[#D4A855]" />
              <span>Báo cáo vận hành</span>
            </button>
          ) : (
            <Link href="/new" className="flex items-center gap-2 bg-white/[0.05] border border-white/10 hover:bg-white/10 px-4 py-2.5 rounded-full text-xs font-bold text-white transition-all">
              <PlusCircle className="w-4 h-4 text-[#D4A855]" />
              <span>Tạo đề xuất mới</span>
            </Link>
          )}
          <Link href="/fleet" className="flex items-center gap-2 bg-white/[0.05] border border-white/10 hover:bg-white/10 px-4 py-2.5 rounded-full text-xs font-bold text-white transition-all">
            <Car className="w-4 h-4 text-[#D4A855]" />
            <span>Xem tình hình xe</span>
          </Link>
          <Link href="/history" className="flex items-center gap-2 bg-white/[0.05] border border-white/10 hover:bg-white/10 px-4 py-2.5 rounded-full text-xs font-bold text-white transition-all">
            <Clock className="w-4 h-4 text-[#D4A855]" />
            <span>Lịch sử chuyến đi</span>
          </Link>
          {pendingCount > 0 && ['dept_head', 'tcth'].includes(user?.role || '') && (
            <Link href="/approve" className="flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 px-4 py-2.5 rounded-full text-xs font-bold text-[#D4A855] transition-all animate-pulse">
              <CheckSquare className="w-4 h-4 text-[#D4A855]" />
              <span>Phê duyệt ngay ({pendingCount})</span>
            </Link>
          )}
          {['admin', 'tcth'].includes(user?.role || '') && (
            <Link href="/admin/requests" className="flex items-center gap-2 bg-violet-500/10 border border-violet-500/20 hover:bg-violet-500/20 px-4 py-2.5 rounded-full text-xs font-bold text-[#D4A855] transition-all">
              <BarChart3 className="w-4 h-4 text-[#D4A855]" />
      ...
      <div className="mb-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
        <div className="relative rounded-3xl p-5 bg-gradient-to-r from-[#0c182b] via-[#102340] to-[#0c182b] border border-cyan-500/20 shadow-xl overflow-hidden flex items-center justify-between">
          ...
          <div>
            <p className="text-[10px] text-[#D4A855] font-bold uppercase tracking-wider">Hệ sinh thái số VietinBank</p>
            <h3 className="text-sm font-bold text-white mt-0.5">Số hóa quy trình điều xe Nam Sài Gòn</h3>
            <div className="flex gap-2 mt-3">
              {user && ['tcth', 'admin'].includes(user.role) && (
                <button onClick={() => setShowDirectTaskModal(true)} className="text-[10px] bg-cyan-500/20 text-[#D4A855] border border-cyan-500/30 px-3 py-1 rounded-full hover:bg-cyan-500/30 font-bold flex items-center gap-1 cursor-pointer">
                  <Send className="w-3 h-3 text-[#D4A855]" /> Giao việc trực tiếp
                </button>
              )}
              <Link href="/new" className="text-[10px] bg-white/[0.06] text-white font-bold border border-white/10 px-2.5 py-1 rounded-full hover:bg-white/10">
                → Đăng ký nhanh
              </Link>
            </div>
          </div>
          <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center shrink-0">
            <Car className="w-7 h-7 text-[#D4A855]" />
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 animate-slide-up" style={{ animationDelay: '0.12s' }}>
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          <span className="text-xs text-[#9CA3AF] hover:text-white transition-colors cursor-pointer">
            Phân quyền: <strong className="text-[#D4A855]">{roleCfg?.label}</strong>
          </span>
        </div>
        ...
        <div className="bg-[#121929]/90 border border-white/10 rounded-3xl p-5 shadow-2xl backdrop-blur-2xl grid grid-cols-3 gap-y-6 gap-x-2">
          ...
          {user && ['tcth', 'admin'].includes(user.role) && (
            <button onClick={() => setShowDirectTaskModal(true)} className="flex flex-col items-center text-center group cursor-pointer">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-500/25 to-blue-600/25 border border-cyan-500/40 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2 shadow-lg shadow-cyan-500/15">
                <Send className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-[#D4A855] leading-tight">Giao việc trực tiếp</span>
            </button>
          )}
          ...
          {user?.role === 'director' ? (
            <button onClick={() => setShowOpsReport(true)} className="flex flex-col items-center text-center group cursor-pointer">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-emerald-500/25 to-teal-600/25 border border-emerald-500/40 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2 shadow-lg shadow-emerald-500/15">
                <ClipboardList className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-[#D4A855] leading-tight">Báo cáo vận hành</span>
            </button>
          ) : (
            <Link href="/new" className="flex flex-col items-center text-center group">
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#f4c3af]/20 to-[#e0a98b]/20 border border-[#f4c3af]/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
                <PlusCircle className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-white leading-tight">Tạo đề xuất</span>
            </Link>
          )}
          ...
          {user?.role === 'staff' ? (
            <Link href="/history" className="flex flex-col items-center text-center group">
              <div className="w-12 h-12 rounded-2xl bg-cyan-500/15 border border-cyan-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
                <FileText className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-white leading-tight">Đề xuất của tôi</span>
            </Link>
          ) : user?.role !== 'director' ? (
          ...
          {user && ['tcth', 'admin'].includes(user.role) && (
            <Link href="/admin/requests" className="flex flex-col items-center text-center group">
              <div className="w-12 h-12 rounded-2xl bg-violet-500/15 border border-violet-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
                <BarChart3 className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-white leading-tight">Quản lý đề xuất</span>
            </Link>
          )}

          {/* Item 4: Đội xe công tác */}
          <Link href="/fleet" className="flex flex-col items-center text-center group">
            <div className="w-12 h-12 rounded-2xl bg-purple-500/15 border border-purple-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
              <Truck className="w-6 h-6" />
            </div>
            <span className="text-[11px] font-bold text-white leading-tight">Tình hình Đội xe</span>
          </Link>

          {/* Item 5: Giám sát GPS */}
          <Link href="/monitor" className="flex flex-col items-center text-center group">
            <div className="w-12 h-12 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
              <Activity className="w-6 h-6" />
            </div>
            <span className="text-[11px] font-bold text-white leading-tight">Giám sát GPS</span>
          </Link>

          {/* Item 6: Lịch sử điều xe */}
          <Link href="/history" className="flex flex-col items-center text-center group">
            <div className="w-12 h-12 rounded-2xl bg-amber-500/15 border border-amber-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
              <Clock className="w-6 h-6" />
            </div>
            <span className="text-[11px] font-bold text-white leading-tight">Lịch sử điều xe</span>
          </Link>

          {/* Item 7: Admin — Chỉ hiện cho Admin */}
          {user?.role === 'admin' && (
            <Link href="/admin" className="flex flex-col items-center text-center group">
              <div className="w-12 h-12 rounded-2xl bg-rose-500/15 border border-rose-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
                <Settings className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-white leading-tight">Quản trị Admin</span>
            </Link>
          )}
          ...
          {user?.role === 'driver' && (
            <Link href="/driver" className="flex flex-col items-center text-center group">
              <div className="w-12 h-12 rounded-2xl bg-indigo-500/15 border border-indigo-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
                <Compass className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-white leading-tight">Nhiệm vụ tài xế</span>
            </Link>
          )}
          ...
          <button onClick={() => setShowSafetyModal(true)} className="flex flex-col items-center text-center group cursor-pointer">
            <div className="w-12 h-12 rounded-2xl bg-blue-500/15 border border-blue-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
              <ShieldCheck className="w-6 h-6" />
            </div>
            <span className="text-[11px] font-bold text-white leading-tight">Quy định an toàn</span>
          </button>
          ...
          {user?.role === 'admin' && (
            <button onClick={() => setShowKmModal(true)} className="flex flex-col items-center text-center group cursor-pointer">
              <div className="w-12 h-12 rounded-2xl bg-teal-500/15 border border-teal-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
                <BarChart3 className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-white leading-tight">Thống kê km</span>
            </button>
          )}
          ...
          {user && ['admin', 'tcth', 'dept_head'].includes(user.role) && (
            ...
            >
              <div className="w-12 h-12 rounded-2xl bg-green-500/15 border border-green-500/30 flex items-center justify-center text-[#D4A855] group-hover:scale-110 transition-transform mb-2">
                <Download className="w-6 h-6" />
              </div>
              <span className="text-[11px] font-bold text-white leading-tight">Xuất Excel</span>
            </button>
          )}
      ...
      <div className="animate-slide-up" style={{ animationDelay: '0.15s' }}>
        <div className="flex items-center justify-between mb-3 px-1">
          ...
          {allRequests.length > 4 && (
            <Link href="/history" className="flex items-center gap-1 text-xs text-[#D4A855] font-bold hover:underline">
        ...
        {recentRequests.length === 0 ? (
          <GlassCard className="p-8 text-center bg-[#121929]/80 border-white/10 rounded-3xl">
            <Car className="w-10 h-10 text-[#9CA3AF] mx-auto mb-2" />
            <p className="text-sm font-bold text-[#9CA3AF]">Chưa có đề xuất nào</p>
          </GlassCard>
        ) : (
                    ...
                    {canEdit && (
                      <div className="flex items-center gap-2 mt-2.5 pt-2.5 border-t border-white/[0.06]">
                        <span
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-[#D4A855] text-[11px] font-bold hover:bg-cyan-500/20 transition-all cursor-pointer"
                        >
                        ...
                        <span
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-red-500/10 border border-red-500/20 text-[#D4A855] text-[11px] font-bold hover:bg-red-500/20 transition-all cursor-pointer"
                        >
                          <Trash2 className="w-3 h-3" />
                          Xóa
                        </span>
                        <span className="flex-1" />
                        <span className="text-[9px] text-[#9CA3AF] italic">Đang chờ duyệt — có thể sửa/xóa</span>
                      </div>
                    )}
                  </GlassCard>
                </Link>
      ...
      {showSafetyModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#090d16]/90 backdrop-blur-xl">
          <div className="relative w-full max-w-md bg-[#121929] border border-white/10 rounded-3xl p-6 shadow-2xl animate-scale-in">
            <button onClick={() => setShowSafetyModal(false)} className="absolute top-4 right-4 text-[#9CA3AF] hover:text-white">
              <X className="w-5 h-5" />
            </button>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-2xl bg-blue-500/20 flex items-center justify-center text-[#D4A855]">
                <ShieldCheck className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-base font-bold text-white">Quy định Sử dụng Xe công tác</h3>
                <p className="text-xs text-[#9CA3AF]">VietinBank Chi nhánh Nam Sài Gòn</p>
              </div>
            </div>
            <div className="space-y-3 text-xs text-white leading-relaxed max-h-[60vh] overflow-y-auto pr-1">
      ...
      {showKmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#090d16]/90 backdrop-blur-xl">
          <div className="relative w-full max-w-md bg-[#121929] border border-white/10 rounded-3xl p-6 shadow-2xl animate-scale-in">
            <button onClick={() => setShowKmModal(false)} className="absolute top-4 right-4 text-[#9CA3AF] hover:text-white">
              <X className="w-5 h-5" />
            </button>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-2xl bg-teal-500/20 flex items-center justify-center text-[#D4A855]">
                <BarChart3 className="w-6 h-6" />
              </div>
              <div>
                <h3 className="text-base font-bold text-white">Thống kê Kilomet công tác</h3>
                <p className="text-xs text-[#9CA3AF]">Chi nhánh Nam Sài Gòn</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-3 bg-teal-500/10 border border-teal-500/20 rounded-2xl text-center">
                <span className="text-[10px] text-[#9CA3AF] font-bold block">Tổng quãng đường</span>
                <strong className="text-xl font-bold text-[#D4A855]">{totalKm.toLocaleString()} km</strong>
              </div>
              <div className="p-3 bg-cyan-500/10 border border-cyan-500/20 rounded-2xl text-center">
                <span className="text-[10px] text-[#9CA3AF] font-bold block">Chuyến hoàn thành</span>
                <strong className="text-xl font-bold text-[#D4A855]">{stats.completed} chuyến</strong>
              </div>
            </div>
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06] text-xs text-white">
              <p className="font-bold text-white mb-1">💡 Báo cáo chi tiết:</p>

    ...
    <div className="min-h-screen bg-slate-900 text-white pb-28 px-4 pt-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-extrabold text-white">Tình hình Đội xe & Tài xế</h1>
      ...
      {!canManage && (
        <div className="mb-4 p-3 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 flex items-center gap-2.5 text-xs text-[#D4A855]">
      ...
      <div className="flex bg-slate-800/60 border border-white/10 p-1 rounded-xl mb-6">
        <button
          ...
          className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
            activeTab === 'xe' ? 'bg-cyan-500/20 text-[#D4A855] border border-cyan-500/30 shadow-sm' : 'text-[#9CA3AF] hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
            activeTab === 'taixe' ? 'bg-cyan-500/20 text-[#D4A855] border border-cyan-500/30 shadow-sm' : 'text-[#9CA3AF] hover:text-white'
          }`}
        >
          ...
          {stats && (
            <div className="flex gap-2 text-xs mb-4">
              <div className="flex-1 bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{stats.availableVehicles}</div>
                <div className="text-[#9CA3AF] text-[10px]">Sẵn sàng</div>
              </div>
              <div className="flex-1 bg-blue-500/10 border border-blue-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{stats.inUseVehicles}</div>
                <div className="text-[#9CA3AF] text-[10px]">Đang công tác</div>
              </div>
              <div className="flex-1 bg-purple-500/10 border border-purple-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{stats.scheduledTripsToday}</div>
                <div className="text-[#9CA3AF] text-[10px]">Lịch hôm nay</div>
              </div>
              <div className="flex-1 bg-amber-500/10 border border-amber-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{stats.maintenanceVehicles}</div>
                <div className="text-[#9CA3AF] text-[10px]">Bảo trì</div>
              </div>
            </div>
          )}
              ...
              <div key={v.id} style={{ animationDelay: `${i * 100}ms` }} className="animate-in fade-in slide-in-from-bottom-2 duration-500 fill-mode-both">
                ...
                {nextTrip && (
                  <div className="mt-1.5 flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-purple-500/10 border border-purple-500/20">
                    <CalendarClock className="w-3.5 h-3.5 text-[#D4A855] shrink-0" />
                    ...
                    {schedule.length > 1 && (
                      <span className="text-[9px] text-[#D4A855]/60 ml-auto">+{schedule.length - 1} lịch khác</span>
                    )}
                  </div>
                )}
                {canManage && (
                  <div className="mt-2 flex items-center justify-end gap-2">
                    <span className="text-[10px] text-[#9CA3AF]">Đổi trạng thái:</span>
                    <select
                      value={v.status}
                      onChange={(e) => handleVehicleStatusChange(v.id, e.target.value)}
                      className="text-xs bg-white/[0.06] border border-white/10 rounded-lg px-2 py-1 text-white focus:outline-none focus:border-cyan-500/50"
                    >
      ...
      {activeTab === 'taixe' && (
        <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
           <div className="flex gap-2 text-xs mb-4">
              <div className="flex-1 bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{stats?.availableDrivers ?? drivers.filter(d => d.status === 'available').length}</div>
                <div className="text-[#9CA3AF] text-[10px]">Sẵn sàng</div>
              </div>
              <div className="flex-1 bg-blue-500/10 border border-blue-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{stats?.onDutyDrivers ?? drivers.filter(d => d.status === 'on_duty').length}</div>
                <div className="text-[#9CA3AF] text-[10px]">Đang chạy</div>
              </div>
              <div className="flex-1 bg-purple-500/10 border border-purple-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{stats?.scheduledTripsTomorrow ?? 0}</div>
                <div className="text-[#9CA3AF] text-[10px]">Lịch ngày mai</div>
              </div>
              <div className="flex-1 bg-amber-500/10 border border-amber-500/20 rounded-xl p-2.5 text-center">
                <div className="text-[#D4A855] font-bold text-xl">{drivers.filter(d => d.status === 'day_off' || d.status === 'sick_leave').length}</div>
                <div className="text-[#9CA3AF] text-[10px]">Nghỉ phép/ốm</div>
              </div>
            </div>
